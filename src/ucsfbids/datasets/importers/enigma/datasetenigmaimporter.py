...
from ucsfbids.subjects.importers.enigma import SubjectEnigmaImporter
...
class DatasetEnigmaImporter(DatasetImporter):
    def _process_subjects(self, subjects: list[str]):
        ...
        for subject in subjects:
            ...
            self.dataset.subjects[subject].add_importer("Enigma", SubjectEnigmaImporter)
    ...
    def import_subjects(
        self, path: Path, source_patients: list[str], overwrite: bool = False
    ):
        assert self.dataset is not None
        ...
        for subject, source_patient in zip(
            self.dataset.subjects.values(), source_patients
        ):
            if (
                subject in participants_tsv_data["participant_id"].values
                and not overwrite
            ):
                continue
            subject.create_importer("Enigma", self.src_root).execute_import(
                path, source_patient
            )
            subject_frame = pd.DataFrame(
                data=[subject.name], columns=["participant_id"]
            )
