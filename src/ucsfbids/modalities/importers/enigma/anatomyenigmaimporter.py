...
DEFAULT_FILES = [
    ...
    FileSpec(
        "T1w",
        ".json",
        [Path("acpc/T1_orig.json"), Path("acpc/T1.json")],
        copy_command=strip_json,
    ),
]


class AnatomyEnigmaImporter(AnatomyImporter):
    def construct(
        self,
...
Anatomy.default_importers["Enigma"] = AnatomyEnigmaImporter
